...
{block name="title"}Edit Menu{/block}
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1>Edit Menu: {$menu.title|escape}</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="/admin/menus/update/{$menu.id}">
                            ...
                            <div class="col-md-8">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="title" name="title" 
                                           value="{$smarty.session.old_input.title|default:$menu.title|escape}" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="type" name="type" onchange="toggleMenuFields()">
                                        {foreach $menu_types as $key => $label}
                                            <option value="{$key}" 
                                                {if ($smarty.session.old_input.type|default:$menu.type) == $key}selected{/if}>
                                                {$label}
                                            </option>
                                        {/foreach}
                                    </select>
                                ...
                                <div class="mb-3" id="custom-url-field">
                                    ...
                                    <input type="text" class="form-control" id="url" name="url" 
                                           value="{$smarty.session.old_input.url|default:$menu.url|escape}" 
                                    ...
                                    <select class="form-select" id="page_id" name="page_id">
                                        ...
                                        {foreach $pages as $page}
                                            <option value="{$page.id}" 
                                                {if ($smarty.session.old_input.page_id|default:($menu.type == 'page' ? $menu.content_id : '')) == $page.id}selected{/if}>
                                                {$page.title|escape}
                                            </option>
                                        {/foreach}
                                    </select>
                                    ...
                                    <select class="form-select" id="post_id" name="post_id">
                                        ...
                                        {foreach $posts as $post}
                                            <option value="{$post.id}" 
                                                {if ($smarty.session.old_input.post_id|default:($menu.type == 'post' ? $menu.content_id : '')) == $post.id}selected{/if}>
                                                {$post.title|escape}
                                            </option>
                                        {/foreach}
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="css_class" name="css_class" 
                                           value="{$smarty.session.old_input.css_class|default:$menu.css_class|escape}" 
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="location" name="location">
                                        {foreach $locations as $loc}
                                            <option value="{$loc}" 
                                                {if ($smarty.session.old_input.location|default:$menu.location) == $loc}selected{/if}>
                                                {$loc|capitalize}
                                            </option>
                                        {/foreach}
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="parent_id" name="parent_id">
                                        ...
                                        {foreach $parent_menus as $parent}
                                            {if !$parent.parent_id && $parent.id != $menu.id}
                                                <option value="{$parent.id}" 
                                                    {if ($smarty.session.old_input.parent_id|default:$menu.parent_id) == $parent.id}selected{/if}>
                                                    {$parent.title|escape}
                                                </option>
                                            {/if}
                                        {/foreach}
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="position" name="position" 
                                           value="{$smarty.session.old_input.position|default:$menu.position}" min="0">
                                    <div class="form-text">Posizione nella sequenza del menu</div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="target" name="target">
                                        <option value="_self" 
                                            {if ($smarty.session.old_input.target|default:$menu.target) == '_self'}selected{/if}>
                                            Stessa finestra
                                        </option>
                                        <option value="_blank" 
                                            {if ($smarty.session.old_input.target|default:$menu.target) == '_blank'}selected{/if}>
                                            Nuova finestra
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="active" name="active" 
                                           {if ($smarty.session.old_input.active|default:$menu.active) == '1'}checked{/if}>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Aggiorna Menu
                            </button>
                        </div>
                    </form>
                </div>
